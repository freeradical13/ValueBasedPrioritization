...
Then, a set of \textbf{comparable prediction models} $R(D(A))$ is applied to each $D(A)$ (e.g. exponential smoothing\cite{hyndman2018forecasting}\textsuperscript{,}\footnote{\url{https://otexts.com/fpp2/expsmooth.html}}, ARIMA\cite{hyndman2018forecasting}\textsuperscript{,}\footnote{\url{https://otexts.com/fpp2/arima.html}}, linear regression\cite{hyndman2018forecasting}\textsuperscript{,}\footnote{\url{https://otexts.com/fpp2/regression.html}}, etc.):
...
The models are compared using \textbf{model selection} (or forecasting)\cite{hyndman2018forecasting,wit2012all,zucchini2000introduction}\textsuperscript{,}\footnote{\url{https://otexts.com/fpp2/selecting-predictors.html}} using a model selection algorithm \inlineequation[model-selection]{L(R(D(A)))} (e.g. smallest Akaike's Information Criterion [AIC], smallest Corrected AIC [AICc], smallest Bayesian Information Criterion [BIC], smallest cross-validation, largest adjusted coefficient of determination [$\bar{R^2}$], etc.).

For each action, $L(R(D(A)))$ produces the \textbf{best fitting model} $M(A(t))$ (or a model that's an average of multiple models\cite{clemen1989combining}\textsuperscript{,}\footnote{\url{https://otexts.com/fpp2/combinations.html}}).
...
Run each comparable prediction model $R_i(D(A))$. For example, see Figure \ref{fig:neoplasmregs} for exponential smoothing functions using Holt's linear trend method for \textit{Malignant neoplasms}\footnote{\texttt{python3 -m vbp.run predict UnderlyingCausesOfDeathUnitedStates --do-not-obfuscate -p 10 "Malignant neoplasms"}}.
...
Use each $M(A(t_F))$ to calculate the predicted value for $t_F$ (setting negative values to 0).
