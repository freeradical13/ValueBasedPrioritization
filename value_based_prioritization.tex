...
A \textbf{value system} $V$ generates a \textbf{goal} $G(t)$ (for some future time $t$) and a set of \textbf{mutually exclusive potential future actions} $A(t) = \{A_1(t), \ldots, A_N(t)\}$ where $N > 1$.
...
The models are compared (e.g. using adjusted $r^2$) and the \textbf{best fitting model} $M(R(D(A)))$ is selected. Note that $M(R(D(A)))$ may be different for each action.
