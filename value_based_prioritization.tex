...
\item $S_3(A_i) = \begin{cases}\text{0.1} & \mbox{if heteroscedastic} \\ \text{1} & \mbox{if homoscedastic} \\ \end{cases}$
                \newline\newline
                Essentially remove actions with heteroscedastic $M(A(t))$.
...
$D(A)$ for each action is the time-series data of number of deaths per year. For example, for \textit{Malignant neoplasms}:
...
\bottomrule
\end{tabular}
\caption{Deaths per year for \textit{Malignant neoplasms}}
\label{table:daa1}
\end{table}
