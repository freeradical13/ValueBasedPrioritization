...
The models are compared using \textbf{model selection}\cite{wit2012all,zucchini2000introduction} \inlineequation[model-selection]{L(R(D(A)))} (e.g. adjusted $r^2$, log-likelihood, AIC, BIC, ANOVA, cross-validation, etc.).
...
\item \eqref{model-selection} $L(R(D(A))) = $ highest adjusted $r^2$ ($\bar{R^2}$), lowest log-likelihood, highest AIC, and highest BIC.
\end{itemize}
...
\item $S_2(A_i) = \begin{cases}\text{0.1} & \mbox{if } \bar{R^2} < 0.1 \\ \text{1} & \mbox{if } \bar{R^2} \ge 0.1 \\ \end{cases}$
                \newline\newline
                Essentially remove actions with poorly predicted $B(A_i(t_F))$.
\item $S_3(A_i) = \begin{cases}\text{0.1} & \mbox{if heteroscedastic} \\ \text{1} & \mbox{if homoscedastic} \\ \end{cases}$
                \newline\newline
                Essentially remove actions with heteroscedastic $M(A(t))$ (using the Breusch-Pagan Lagrange Multiplier test).
\item $S_4(A_i) = \left(1 - \frac{AverageAge(A_i)}{MaxAge(A(t))}\right)$
                \newline\newline
...
\item $S_6(A_i) = \begin{cases}\text{0.1} & \mbox{if primarily political} \\ \text{1} & \mbox{otherwise} \\ \end{cases}$
                \newline\newline
...
Create a table listing all actions as rows and all \textit{manually calculated} scale functions as columns\footnote{For the example scale functions, this is only $S_6(A_i)$.}\textsuperscript{,}\footnote{\texttt{python3 -m vbp.run scale\char`_functions -t excel -o scale\char`_functions.xslx -n "Scale Values" -p "Eliminate: " UnderlyingCausesOfDeathUnitedStates S6}}:
...
$D(A)$ for each action is the time-series data of number of deaths per year per 100,000 of population (\enquote{Crude Rate}). For example, for \textit{Malignant neoplasms}:
