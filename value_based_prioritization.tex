...
Create a table listing all actions as rows and all \textit{manually calculated} scale functions as columns\footnote{For the scale functions above, this is only $S_6(A_i)$}\textsuperscript{,}\footnote{\texttt{python3 -m vbp.run scale\char`_functions -t excel -o scale\char`_functions.xslx -n "Scale Values" -p "Eliminate: " UnderlyingCausesOfDeathUnitedStates S6}}:
