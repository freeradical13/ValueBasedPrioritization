...
\begin{itemize}
\item $S_1(A) = 1$: Required scale function.
\item $S_2(A)$: Likelihood of Success. If primarily political then 0.1, else 1 (i.e. unpredictable).
\item $S_3(A) = \left(1 - \frac{AvgAge(A)}{MaxAge}\right)$
                \newline\newline
                Scale towards younger people as they have more to lose.
\item $S_4(A) = \begin{cases}\text{0} & \mbox{if } L(R(D(A))) < 0.1 \\ \text{1} & \mbox{if } L(R(D(A))) \ge 0.1 \\ \end{cases}$
                \newline\newline
                Remove actions with poorly predicted $B(A(t_F))$.
\item $S_5(A) = \frac{f(A)-f_{min}}{f_{max}-f_{min}}$
                \newline\newline
                $f(A) = M'(A(t_F))$
                \newline\newline
                Scale by the relative rate of change of an action's predicted rate of death; take the derivative of $M(A)$ and evaluate it with the predicted value and min-max normalize\footnote{\url{https://en.wikipedia.org/wiki/Feature_scaling}} into $[0,1]$.
\end{itemize}
