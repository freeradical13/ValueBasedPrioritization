...
For each model, calculate the p-value of the F-statistic, $\bar{R^2}$, log-likelihood, AIC, BIC, and the Breusch-Pagan Lagrange Multiplier test. Filter out models whose p-value of the F-statistic is $> 0.05$ or p-value of Breusch-Pagan is $< 0.05$ and choose the model that has the lowest log-likelihood, highest AIC, and highest BIC. If no models are found, perform the same procedure independent of the p-value tests (the scale functions $S_2(A_i)$ and $S_3(A_i)$ will scale down the chosen model later). If no models are found, choose the degree-1 regression based on the assumption that most disease trends do not tend to change in higher-order ways.
