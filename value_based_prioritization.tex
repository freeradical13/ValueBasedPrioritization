...
Then, a set of \textbf{comparable prediction models} $R(D(A))$ is applied to each $D(A)$ (e.g. linear regression with different degrees\cite{berk2004regression}):
...
The models are compared using \textbf{model selection}\cite{wit2012all,zucchini2000introduction} \inlineequation[model-selection]{L(R(D(A)))} (e.g. Akaike's Information Criterion [AIC], Corrected AIC [AICc], Bayesian Information Criterion [BIC], etc.).
