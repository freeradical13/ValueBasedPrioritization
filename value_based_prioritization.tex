...
\item $S_2(A_i) = \begin{cases}\text{0.1} & \mbox{if primarily political} \\ \text{1} & \mbox{otherwise} \\ \end{cases}$
                \newline\newline
...
\item $S_4(A_i) = \begin{cases}\text{0} & \mbox{if } \bar{R^2} < 0.1 \\ \text{1} & \mbox{if } \bar{R^2} \ge 0.1 \\ \end{cases}$
                \newline\newline
...
Create a table listing all actions as rows and all \textit{manually calculated} scale functions as columns. For example\footnote{\texttt{python -m vbp.run scale\char`_functions -t excel -o scale\char`_functions.xslx -n "Scale Values" -p "Eliminate: " UnderlyingCausesOfDeathUnitedStates S2}}:
