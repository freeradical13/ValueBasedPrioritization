...
\begin{itemize}
  ...
  \item \eqref{prediction-models} $R(D(A)) = $ exponential smoothing functions using Holt's linear trend method\footnote{\scriptsize{\url{https://otexts.com/fpp2/holt.html}}}\textsuperscript{,}\footnote{\scriptsize{\url{https://otexts.com/fpp2/ets.html}}}\textsuperscript{,}\footnote{\scriptsize{\url{https://www.statsmodels.org/dev/examples/notebooks/generated/exponential_smoothing.html}}}:
    \begin{equation*}
      \begin{gathered}
...
Run each comparable prediction model $R_i(D(A))$. For example, for \textit{Ischemic heart diseases}\footnote{\texttt{python3 -m vbp.run predict UnderlyingCausesOfDeathUnitedStates --do-not-obfuscate -p 10 "Ischemic heart diseases"}}:

\begin{figure}[H]
  \centering
  \includegraphics[width=0.5\textwidth]{Ischemic_heart_diseases_ets.png}
  \caption{Exponential smoothing functions $ETS(A,*,N),\phi=0.98$ using Holt's linear trend method for \textit{Ischemic heart diseases}}\label{fig:neoplasmregs}
\end{figure}
...
\begin{table}[H]
  ...
  \begin{tabular}{lcc}
    ...
    \midrule
      $ETS(A,A,N)$      & 56.78          & 105.01          \\
      $ETS(A,A_d,N)$    & 63.14          & 105.33          \\
      $\bm{ETS(A,M,N)}$ & \textbf{51.99} & \textbf{104.74} \\
      $ETS(A,M_d,N)$    & 57.32          & 105.57          \\
    \bottomrule
  \end{tabular}
    ...
    \begin{table}[H]
      ...
      \begin{tabular}{clc}
        \toprule
          $k$ & Action                  & $Z(t_F)$ \\
        \midrule
          1   & Eliminate: TODO         & TODO     \\
          2   & Eliminate: TODO         & TODO     \\
        \bottomrule
      \end{tabular}
      \caption{$Z(t_F)$ table for the minimally grouped causes of death for the World}
      \label{table:ztable2}
    \end{table}
\end{enumerate}
